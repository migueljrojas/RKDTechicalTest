...
async function getHolidays(country) {
  ...
  let holidaysList = await fetch(
    `./holidays/${
      countryCode === "us" || countryCode === "cr" ? countryCode : "int"
    }.json`
  );
	holidaysList = await holidaysList.json();
  return holidaysList;
}

function isHoliday(date, holidays) {
	return Object.keys(holidays).includes(date);
}

// Make calendar
async function makeCalendar(dateStart, dateLength, country) {
    ...
    if (!isLast) {
      ...
      if (dateString + "/" + tempYear === today) {
        $day_cell.classList.add("today");
			...
			// Check if Day is holiday and adds class name for styling
      if (isHoliday(dateString, holidays)) {
				$day_cell.classList.add("holiday");
				$day_cell.dataset.holiday = holidays[dateString][0].name;

				$day_cell.addEventListener('click', (e) => {
					e.stopImmediatePropagation();
					alert(e.target.dataset.holiday);
				});
			}
      ...
      fillEmptyMonth(tempYear, tempMonth, tempDay + tempCount, monthLimit, holidays);
    }
...
function fillEmptyMonth(year, month, start, length, holidays) {
  var firstDay = firstDayOfMonth(year, month);
  var monthId = monthName[month] + "_" + year;
  var $month = document.getElementById(monthId);
  var loopLen = length;
  ...
  for (var i = start - 1; i < loopLen; i++) {
	...
	var days = [...$month.querySelectorAll('li')];

	for (const day of days) {
		if (isHoliday(`${day.innerText}/${month}`, holidays)) {
			day.classList.add('holiday');
			day.dataset.holiday = holidays[`${day.innerText}/${month}`][0].name;
  ...
  formElem.addEventListener(
    ...
    function (e) {
      ...
      if (!errorsLen) {
        ...
        cal.scrollTop = 0;
        var [startDate, dateLength, countryCode] = inputArray;
        makeCalendar(startDate.value, dateLength.value, countryCode.value);
      }
    },
    false
  );
